...
export function CandidateDashboard() {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        const [catsRes, testsRes, badgesRes, attemptsRes, followersRes] = await Promise.all([
        ...
        const badgeList: Badge[] = Array.isArray(badgesRes.data) ? badgesRes.data : badgesRes.data.results || [];
        ...
        const attemptList: Attempt[] = Array.isArray(attemptsRes.data) ? attemptsRes.data : attemptsRes.data.results || [];
        setAttempts(attemptList.slice(0, 5));
        
        const followerData = followersRes.data;
        const fCount = followerData.count ?? (Array.isArray(followerData) ? followerData.length : followerData.results?.length || 0);
        setFollowersCount(fCount);
        if (fCount > 0) {
          const resultsList = followerData.results || followerData;
          if (resultsList.length > 0) {
            setLatestFollowerCompany(resultsList[0].company_name);
          }
        }
